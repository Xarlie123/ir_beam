...
# 📌 Modelo DNN para inferencia de parámetros
class DNNRegressor(nn.Module):
    ...
    def forward(self, x):
        x = x.view(x.size(0), -1)  # Aplanar el tensor de entrada
...
class DNNTrainer:
    def __init__(self, dataframe, img_size=64, batch_size=16, learning_rate=0.001, weight_decay=1e-5):
        ...
        self.input_size = img_size * img_size  # Flattened image size
        ...
        self.model = DNNRegressor(self.input_size).to(self.device)
    ...
    def plot_ellipse(self, predicted_params, true_params):
        ...
        axes[0].imshow(pred_gaussian, cmap='jet')
        axes[0].set_title('Predicted Intensity')
        axes[1].imshow(true_gaussian, cmap='jet')
        axes[1].set_title('True Intensity')
        plt.show()

    def save_model(self, path="dnn_regressor.pth"):
        torch.save(self.model.state_dict(), path)

    def load_model(self, path="dnn_regressor.pth"):
